...
error:
    if (chunk->chunk_data) {
        free(chunk->chunk_data);             //fix for chunk_data
        chunk->chunk_data = NULL;
    }
    return 1;
}
...
int load_png(const char *filename, struct image **img)
{
    ...
    // Read all PNG chunks
    for (;!read_png_chunk(input, current_chunk); current_chunk = malloc(sizeof(struct png_chunk)))                  //fix bug: freeing the current chunk to avoid bad leaks
    {
        chunk_idx++;
        ...
        if (is_chunk_idat(current_chunk)) {
            ...
            free(idat_chunk);
            continue; //fix attempt for memory leak
        }
        goto error;     //case where not recognized chunk
    }
 ...
 success:
    ...
    if (plte_chunk) {                   //bugfix: adding free of data
    ...
    if (ihdr_chunk) {
        if (ihdr_chunk->chunk_data) {
            free(ihdr_chunk->chunk_data);           //bugfix, added
        }
        free(ihdr_chunk);
    }

    //bugfix: added the whole section
    if (iend_chunk) {
